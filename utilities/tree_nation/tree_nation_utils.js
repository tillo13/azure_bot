    ...
    while (attempt < maxRetries) {
      try {
        ...
        if (response.data.status === 'ok') {
          // Pass both species and environmentFlag to createDetailResponse
          return {
            userMessage: createDetailResponse(response.data, species, environmentFlag),
...
async function getAvailableSpecies(projectId) {
  ...
  async function getRandomAvailableSpecies(projectIds) {
    try {
      ...
      for (const projectId of projectIds) {
        ...
        allAvailableSpecies.push(...availableSpecies.map(species => ({ ...species, projectId }))); // Attach projectId to each species
      }
      ...
      // Randomly pick an available species and its associated project
  ...
  function createDetailResponse(apiResponse, speciesObject, environmentFlag) { // Now also takes environmentFlag
    ...
    let projectName = 'the chosen location';
    if (environmentFlag === 'TEST') {
      projectName = speciesObject.projectId === 3 ? 'La Pedregoza' :
                    speciesObject.projectId === 41 ? 'Usambara Biodiversity Conservation' :
                    projectName;
    }
  
    return `A tree has been planted successfully${environmentFlag === 'TEST' ? ` in <b>${projectName}</b>` : ''}! Here are the details:\n\n${treeDetails}`;
  }
