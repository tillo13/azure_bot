...
class EchoBot extends ActivityHandler {
    ...
    async processMessage(context, next) {
        let chatMessagesUser = await this.getChatMessages(context);
        const chatResponse = await this.getChatResponse(context, chatMessagesUser);
        ...
        this.handleResponds(context, chatResponse);
        console.log(`\n\n\n****BOT_ROUTER.JS current channelData:\n\n${JSON.stringify(context.activity.channelData, null, 2)}`);
        await next();
    }

    getThreadTS(context) {
        return context.activity.channelData && context.activity.channelData.SlackMessage && context.activity.channelData.SlackMessage.event ?
    ...
    async getChatMessages(context) {
        const current_thread_ts = this.getThreadTS(context);
    ...
    async getChatResponse(context, chatMessagesUser) {
    ...
    handleResponds(context, chatResponse) {
        if (isFromSlack(context)) {
            console.log('\n***BOT_ROUTER.JS: letMeCheckFlag is: ', chatResponse.letMeCheckFlag);
        } else {
            this.defaultRespond(context, chatResponse);
        }
    }

    async defaultRespond(context, chatResponse) {
        const replyText = `default_router: ${chatResponse.assistantResponse}`;
        const replyActivity = MessageFactory.text(replyText);
