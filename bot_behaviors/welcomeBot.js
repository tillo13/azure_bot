...
class WelcomeBot extends ActivityHandler {
    ...
    constructor(userState) {
        ...
        this.onMessage(async (context, next) => {
            ...
            const chatMessagesUser = await this.chatMessagesProperty.get(context, false);
            ...
            if (didBotWelcomedUser === false) {
                ...
                // Setup OpenAI system role
                const systemRole = { "role": "system", "content": "You are a helpful assistant. You will talk like an old cowboy from montana"};
                const messages = [systemRole];
                await context.sendActivity('GPT: Setting up system role.');
                await this.chatMessagesProperty.set(context, messages);
                ...
                console.log(chatMessagesUser);
                let messages = chatMessagesUser;
...
//    console.log(`Messages: ${messages.map((m) => m.content).join("\n")}`);
    ...
    for (const choice of result.choices) {
      console.log("chat completion choices")
