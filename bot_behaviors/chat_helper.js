...
async function chatCompletion(chatTexts, roleMessage, channelId, isActiveThread) {
	...
	if (result && result.choices[0]?.message?.content) {
		...
		if (usedGPT4) {
			assistantResponse += " ||||Response enhanced with Weaviate and provided by GPT-4||||";
		} else {
			//if we want to add the default, uncomment this; assistantResponse += " (|||Response generated by GPT-3.5 Turbo)";
		}
		if (letMeCheckFlag) {
			let newMessages = [{
				role: 'assistant',
